...
Get `SERVER_ID` and `AGENT_SECRET` from the admin dashboard → Servers → info icon.

### 5. Configure VLESS+Reality (optional)

Install 3x-ui on the VPN server, create a VLESS+Reality inbound, then update the server via the admin dashboard → Servers → edit (pencil icon). Fill in the x-ui and VLESS fields.
